        ...
        <form class="w-xl" action="{{ route('weapons.update', $weapon->id) }}" method="POST">
            ...
            <div class="bg-sky-800 p-3 m-6 md:w-4/5 rounded-xl">
                <div class="flex flex-row justify-between m-3">
                    ...
                    <input class="border-1 border-stone-950 rounded-xl p-1" type="text" name="name" value="{{ $weapon->name }}" required>
                </div>

                <div class="flex flex-row justify-between m-3">
                    <label class="text-lg" for="baseDamage">Base Damage: </label>
                    <input class="border-1 border-stone-950 rounded-xl p-1" type="text" name="baseDamage" value="{{ $weapon->baseDamage }}" required>
                </div>

                <div class="flex flex-row justify-between m-3">
                    <label class="text-lg" for="knockback">Knockback: </label>
                    <input class="border-1 border-stone-950 rounded-xl p-1" type="text" name="knockback" value="{{ $weapon->knockback }}" required>
                </div>

                <div class="flex flex-row justify-between m-3">
                    <label class="text-lg" for="attackSpeed">Attack Speed: </label>
                    <input class="border-1 border-stone-950 rounded-xl p-1" type="text" name="attackSpeed" value="{{ $weapon->attackSpeed }}" required>
                </div>
